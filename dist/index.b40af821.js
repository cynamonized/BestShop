function Calculator(leftParent, rightParent) {
    ...
    this.inputFields = {
        productsQuantity: leftParent.querySelector(".input__number--products-quantity"),
        estimatedMonths: leftParent.querySelector(".input__number--estimated-months"),
    ...
    this.SelectionList = leftParent.querySelector(".calculator__columns .form-select-options");
    ...
    this.totalRowValueLabel = this.totalRowContainer.querySelector(".total__price");
...
Calculator.prototype.UpdateTotal = function() {
    ...
    if (products >= 0) {
        ...
        this.resultRows[0].querySelector(".price--products").innerText = "$" + productsPrice;
        this.resultRows[0].querySelector(".math--products").innerText = products + "* $" + this.prices.product;
    }
    if (products >= 1) this.resultRows[0].style.display = "flex";
    else this.resultRows[0].style.display = "none";
    if (months >= 0) {
        monthsPrice = months * this.prices.month;
        this.resultRows[1].querySelector(".price--months").innerText = "$" + monthsPrice;
        this.resultRows[1].querySelector(".math--months").innerText = months + "* $" + this.prices.month;
    }
    if (months >= 1) this.resultRows[1].style.display = "flex";
    else this.resultRows[1].style.display = "none";
    // Updating selectionlist
    let SelectionListValue = this.inputFields.package.querySelector("p").innerText;
    if (SelectionListValue == "Basic" || SelectionListValue == "Professional" || SelectionListValue == "Premium") this.resultRows[2].querySelector(".math--package").innerText = SelectionListValue;
    else this.resultRows[2].querySelector(".math--package").innerText = "";
    // This is crap too (2)
    switch(SelectionListValue){
        case "Basic":
            this.resultRows[2].style.display = "flex";
            this.resultRows[2].querySelector(".price--package").innerText = "$" + this.prices.packageBasic;
            currentPackage = this.prices.packageBasic;
            break;
        case "Professional":
            this.resultRows[2].style.display = "flex";
            this.resultRows[2].querySelector(".price--package").innerText = "$" + this.prices.packageProfessional;
            currentPackage = this.prices.packageProfessional;
            break;
        case "Premium":
            this.resultRows[2].style.display = "flex";
            this.resultRows[2].querySelector(".price--package").innerText = "$" + this.prices.packagePremium;
    ...
    if (this.accountingChecked == true) accounting = this.prices.accounting;
    if (this.terminalChecked == true) terminal = this.prices.terminalRental;
    //Updating totals
    this.totalRowValue = productsPrice + monthsPrice + currentPackage + accounting + terminal;
    this.totalRowValueLabel.innerText = "$" + this.totalRowValue;
    //Udpating total visibility
    if (this.validateTotalVisibility() > 0) this.totalRowContainer.style.display = "flex";
    else this.totalRowContainer.style.display = "none";
};
Calculator.prototype.valueValidation = function(e) {
    const value = e.currentTarget.value;
    if (value < 0 && value != "") e.currentTarget.value = 0;
};
Calculator.prototype.UpdateCheckboxes = function(e) {
    ...
    if (checkbox.checked == true && checkbox.id == "accounting") {
        ...
        this.resultRows[3].querySelector(".values__price").innerText = "$" + this.prices.accounting;
    ...
    if (checkbox.checked == true && checkbox.id == "terminal") {
        ...
        this.resultRows[4].querySelector(".values__price").innerText = "$" + this.prices.terminalRental;
...
Calculator.prototype.validateTotalVisibility = function() {
    ...
    this.resultRows.forEach(function(element) {
        if (element.style.display == "flex") visibleRowsCounter++;
    });
...
Calculator.prototype.addEventListeners = function() {
    ...
    this.SelectionListElements.forEach(function(listElement) {
        listElement.addEventListener("click", function() {
            ...
            calculatorThis.inputFields.package.querySelector("p").innerText = listElementValue;
            calculatorThis.inputFields.package.querySelector("p").style.color = "black";
...
document.addEventListener("DOMContentLoaded", function() {
    let rightParentElement = document.querySelector(".calculator__columns .columns__right-column");
...
//# sourceMappingURL=index.b40af821.js.map
